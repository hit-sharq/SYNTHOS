...
export async function POST(_req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    if (!job) return NextResponse.json({ error: Errors.resources.jobNotFound }, { status: 404 })
    if (job.status !== "approved") return NextResponse.json({ error: Errors.validation.invalidStatus }, { status: 400 })
