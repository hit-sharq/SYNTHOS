...
export default function TalentTasksPage() {
  ...
  return (
    <PageWrap>
      <PageHead eyebrow="Talent" title="Tasks" desc="Action items from your project contact reports." />
      {tasks.length === 0 ? (
        <Empty title="No tasks yet" hint="Tasks appear here after contact reports are generated for your projects." />
      ) : (
        <StaggerContainer>
          <div className="stack gap-2">
            {tasks.map((t) => (
              <RevealOnScroll key={t.id}>
                <Link href={`/dashboard/projects/${t.projectId}`} className="panel-soft" style={{ padding: 16, display: "flex", alignItems: "center", gap: 12, textDecoration: "none", color: "inherit" }}>
